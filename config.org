...
#+BEGIN_SRC emacs-lisp
  (use-package exwm
    ...
    :config
      ...
      (setq exwm-randr-workspace-monitor-plist '(1 "VGA-1" 3 "VGA-1"))
      (add-hook 'exwm-randr-screen-change-hook
            (lambda ()
              (start-process-shell-command
               "xrandr" nil "xrandr --output DP-3 LVDS-1 --left-of DP-1 --auto")))
      (exwm-randr-enable)
...
*** functions for start processes
#+begin_src emacs-lisp
      (defun monitor/brightness-up ()
        (interactive)
        (start-process "brightness-up" nil "light" "-A" "7")
        (message "XF86MonBrightnessUp key pressed"))
  
      (defun monitor/brightness-down ()
        (interactive)
        (start-process "brightness-down" nil "light" "-U" "7")
        (message "XF86MonBrightnessDown key pressed!"))
#+end_src
*** Keybindings to related processes
#+begin_src emacs-lisp
  (global-set-key (kbd "<XF86MonBrightnessUp>") 'monitor/brightness-up)
  (global-set-key (kbd "<XF86MonBrightnessDown>") 'monitor/brightness-down)
#+end_src
